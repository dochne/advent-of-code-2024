...
STDIN.read.lines(chomp: true)
    ...
    .yield_self do |grid|
      values = dijkstra_all_paths(Node.new(grid.find("S"), Vector[1, 0]), DIRECTIONS.map{|d| Node.new(grid.find("E"), d)}) do |node|
        result = []
    ...
    shortest_route = values.map{|(k, v)| v.distance}.min
    shorest_route_cells = values.values.reduce([]) do |acc, route|
        next acc if route.distance != shortest_route
        acc += route.path.map{|node| node.cell}
    end

    Set.new(shorest_route_cells).length
    end
    .tap{p(_1)}
